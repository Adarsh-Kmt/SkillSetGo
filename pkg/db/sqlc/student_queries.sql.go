...
FROM job_table JOIN company_table
                    ON job_table.company_id = company_table.company_id
  WHERE (COALESCE(array_length($2::VARCHAR[], 1), 0) = 0 OR salary_tier NOT IN (SELECT unnest($2)))
  AND (COALESCE(array_length($3::VARCHAR[], 1), 0) = 0 OR job_role NOT IN (SELECT unnest($3)))
  AND NOW() < apply_by_date
  AND (COALESCE(array_length($4::INT[], 1), 0) = 0 OR job_id <> ANY($4))
